...
edu_by_LGA_ratio <- edu_by_LGA %>% mutate(ratio = edu_by_LGA_NA$PeopleSum/edu_by_LGA_Totals$PeopleSum) %>% select(LGA, Ed_level, ratio)
...
box_post_prop <- boxplot(edu_uni_prop2[,2], main = 'Proportion of Post Grad Students per SA1')
length(box_post_prop$out) # 203 outliers, significant drop in outliers when you take into account the proportion
box_under_prop <- boxplot(edu_uni_prop2[,3], main = 'Proportion of Under Grad Students per SA1')
