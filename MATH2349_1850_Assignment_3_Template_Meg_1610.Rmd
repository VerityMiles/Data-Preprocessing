...
#Local Government Area Health Data - Structure, names and glimpse output supressed for brevity. 

head(LGA_health) #No extra columns or rows to trim.
tail(LGA_health) #Note that the last column is for all of Victoria.
dim(LGA_health) #8 rows and 404 columns. Consider selecting only a few of these variables to work with. 
#str(LGA_health) # Most variables are character, even ones that should be numeric, but need to decide which variables to work with first. LGA code is numeric but                 we will be matching the LGA names when joining to the other data set. 
#names(LGA_health) #Variables that will be used are: LGA Name, Travel time to Melbourne, Total fertility rate, Top 5 overseas countries of birth - country 1, Top                  5 ancestries - ancestry 1, People who believe multiculturalism makes life better, People reporting high/very high psychological distress,                      People who live near public transport, Primary Health Network (PHN), Median household income
class(LGA_health) #Class is data frame, as expected.
#glimpse(LGA_health)
```
